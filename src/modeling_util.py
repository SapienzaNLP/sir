...
class SimmatModule(torch.nn.Module):
    ...
    def forward(self, query_embed, doc_embed, query_tok=None, doc_tok=None, mask=None):
        simmat = []
        assert (query_tok is not None and doc_tok is not None) or mask is not None
...
class DRMMLogCountHistogram(torch.nn.Module):
    ...
    def forward(self, simmat, dtoks=None, qtoks=None, mask=None):
        assert (dtoks is not None and qtoks is not None) or mask is not None
